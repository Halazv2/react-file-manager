...
export function DetailsPane() {
  const { selectedNode, renderIcon, renderPreview, renderActions, preview, isPreviewLoading, favoriteIds, toggleFavorite, storageKey, onOpenFile, onDownloadFile, canManage } =
    useFileManagerContext();

  if (renderPreview) {
    return (
      <aside className='flex min-h-0 flex-col overflow-hidden border-l border-black/[0.06] bg-gray-50'>
        <div className='flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain p-4'>{renderPreview(selectedNode)}</div>
      </aside>
    );
  }
    ...
    return (
      <aside className="flex min-h-0 flex-col overflow-hidden border-l border-black/[0.06] bg-gray-50">
        <div className="flex h-full flex-col items-center justify-center gap-3 px-2 text-center text-[13px] text-gray-500">
          {renderIcon?.({ id: "empty", name: "", kind: "folder" }, 56) ??
        ...
        <div className='relative flex min-h-[220px] flex-1 flex-col items-center justify-center rounded-lg bg-gray-100/80 px-4 py-5'>
          {isPreviewLoading ? (
            ...
            <figure className='m-0 flex max-h-full flex-col items-center gap-2.5'>
              ...
              {preview?.kind === "pdf" && <figcaption className='text-[11px] text-gray-500'>First page preview</figcaption>}
            </figure>
          ) : showTextPreview ? (
            ...
            <>
              ...
              <span className='mt-2 text-xs text-gray-500'>{isFolder ? `${childCount} item${childCount === 1 ? "" : "s"}` : (extension || "file").toUpperCase()}</span>
            </>
          )}
        </div>

        <div className='mt-3'>
          <h3 className='m-0 mb-2.5 text-sm font-semibold break-words text-gray-900'>{selectedNode.name}</h3>
          {selectedNode.path && <p className='m-0 -mt-1.5 mb-2.5 text-xs break-words text-gray-500'>{selectedNode.path}</p>}
          <dl className='m-0'>
            <div className='flex justify-between gap-3 border-b border-gray-200/70 py-2 text-xs'>
              <dt className='text-gray-500'>Type</dt>
              <dd className='m-0 font-medium text-gray-900'>{isFolder ? "Folder" : (extension || "file").toUpperCase()}</dd>
            </div>
            {isFolder ? (
              ...
              preview?.pages !== undefined && (
                <div className='flex justify-between gap-3 py-2 text-xs'>
                  ...
                  <dd className='m-0 font-medium text-gray-900'>{preview.pages}</dd>
                </div>
              )
            )}
          </dl>
          ...
          {renderActions ? (
            <div className={cn("mt-3", ROW_TRANSITION, FOCUS_RING)}>{renderActions(selectedNode)}</div>
          ) : isFolder && storageKey ? (
            <button
              type='button'
              className={cn(
              ...
              {favoriteIds.includes(selectedNode.id) ? <StarSolidIcon className='text-amber-500' size={16} /> : <StarIcon size={16} />}
